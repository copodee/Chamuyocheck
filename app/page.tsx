...
const LEGAL_JURISDICTIONS = ['Nacional / Federal', ...ARGENTINA_JURISDICTIONS];
...
export function ChamuyoCheckApp({ leasingPage = false }: { leasingPage?: boolean } = {}) {
  ...
  const [leasingAssetType, setLeasingAssetType] = useState('Automotor / rodado');
  ...
  const legalJurisdictionRef = useRef<HTMLSelectElement | null>(null);
  ...
  async function analyze() {
    ...
    if (selectedCategory === 'argentina-legal-documents' && !legalBranch) {
      ...
      document.querySelector<HTMLButtonElement>('.legalBranchGrid button')?.focus();
      return;
    }
    if (selectedCategory === 'argentina-legal-documents' && !legalJurisdiction) {
      setCategoryError('Elegí la jurisdicción principal del caso antes de continuar.');
    ...
    try {
      ...
      if (selectedCategory === 'leasing-specialist') {
        ...
        if (!file) {
          ...
          if (leasingMonths.trim()) form.append('leasingMonths', leasingMonths);
          if (leasingTna.trim()) form.append('leasingTna', leasingTna);
          form.append('leasingOptionMode', leasingOptionMode);
          if (leasingOptionMode === 'percent' && leasingOptionPercent.trim()) form.append('leasingOptionPercent', leasingOptionPercent);
          else if (leasingOptionAmount.trim()) form.append('leasingOptionAmount', leasingOptionAmount);
          if (leasingGuaranteeCanons.trim()) form.append('leasingGuaranteeCanons', leasingGuaranteeCanons);
          if (leasingStructuringFeePercent.trim()) form.append('leasingStructuringFeePercent', leasingStructuringFeePercent);
        }
      }
      ...
      const historyScore = data.decisionAnswer?.kind === 'leasing-specialist'
        ? calculateLeasingTransparencyScore(buildLeasingTransparencyEvidence({
          ...
          manualFields: file ? {} : {
            ...
            months: leasingMonths ? `${leasingMonths} meses` : '',
            tna: leasingTna ? `${leasingTna}%` : '',
            option: leasingOptionMode === 'percent'
              ? (leasingOptionPercent ? `${leasingOptionPercent}%` : '')
              : leasingOptionAmount,
            structuringFee: leasingStructuringFeePercent ? `${leasingStructuringFeePercent}%` : '',
            guaranteeCanons: leasingGuaranteeCanons,
  ...
  return <div className={`appShell ${leasingPage ? 'leasingAppShell' : ''}`}>
    ...
    <aside className="sidebar">
      <div className="brand">{leasingPage ? <div><a className="leasingSidebarBrand" href="/"><img src="/icon.png" alt="" /><span>LEASING SCORING</span></a><div className="tag">Transparencia financiera del leasing</div></div> : <><div className="shield">✓</div><div><div className="logo">CHAMUYO<span>CHECK</span></div><div className="tag">Finanzas · Estafas · Derecho</div></div></>}</div>
      ...
      {activeView === 'inicio' ? <>
        ...
        <section className="heroGrid heroIntroGrid">
          ...
          <div className="panel inputPanel" id="inicio-form">
            <div ref={categoryRef} tabIndex={-1} className="categoryPicker" aria-labelledby="category-picker-title">
              ...
              {selectedCategory === 'argentina-legal-documents' && <div className="legalBranchPicker">
                ...
                <div className="legalBranchGrid" role="radiogroup" aria-label="Rama del derecho argentino" aria-required="true">
                  {LEGAL_BRANCHES.map((branch) => <button key={branch.id} type="button" role="radio" aria-checked={legalBranch === branch.id} className={`categoryOption ${legalBranch === branch.id ? 'selected' : ''}`} onClick={() => { setLegalBranch(branch.id); setCategoryError(''); setAnalysis(null); }}>
                    <span><strong>{branch.label}</strong><span>{branch.description}</span></span>
                  </button>)}
                </div>
                <label className="legalJurisdictionField" htmlFor="legal-jurisdiction"><b>Jurisdicción principal del caso</b>
                  <select ref={legalJurisdictionRef} id="legal-jurisdiction" value={legalJurisdiction} onChange={(event) => { setLegalJurisdiction(event.target.value); setCategoryError(''); setAnalysis(null); }}>
                    <option value="">Elegí Nación, CABA o provincia</option>
                    {LEGAL_JURISDICTIONS.map((jurisdiction) => <option key={jurisdiction} value={jurisdiction}>{jurisdiction}</option>)}
                  </select>
                </label>
                <p className="legalJurisdictionHelp">Usaremos esta jurisdicción para orientar procedimiento, autoridad, fuentes y plazos. Si intervienen varias, explicalo en la consulta.</p>
              </div>}
              ...
              {selectedCategory === 'leasing-specialist' && <div className="leasingJurisdictionPicker">
                ...
                <label>Tipo de bien<select value={leasingAssetType} onChange={(event) => { setLeasingAssetType(event.target.value); confirmLeasingField('assetType'); }}><option>Automotor / rodado</option><option>Maquinaria o equipo</option><option>Inmueble</option><option>Embarcación</option><option>Aeronave</option><option>Otro bien mueble</option></select></label>
            ...
            {selectedCategory === 'argentina-legal-documents' && <p className="legalJurisdictionHelp"><b>Ya podés escribir tu consulta.</b> La rama y la jurisdicción deben estar elegidas al momento de analizar, pero no bloquean la escritura.</p>}
        ...
        <div className="heroGrid">
          ...
          {analysis.argentinaLegalAnalysis?.applicable && <div className="panel legalResultPanel">
            ...
            <p><b>Jurisdicción:</b> {analysis.argentinaLegalAnalysis.selectedJurisdiction || (analysis.argentinaLegalAnalysis.jurisdiction === 'argentina' ? 'Argentina identificada en el contenido' : 'No especificada')}</p>
