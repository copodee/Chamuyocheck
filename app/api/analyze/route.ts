...
export async function handleAnalyzeRequest(req: Request) {
  try {
    ...
    const leasingVehicleContext = selectedCategory === 'leasing-specialist' && [leasingVehicleDescription, leasingVehicleYear, leasingVehicleCondition, leasingFiscalValuation, leasingPriorYearPatent].some(Boolean)
      ? `Datos del automotor informados por el usuario: condición: ${leasingVehicleCondition === 'new' ? '0 km' : leasingVehicleCondition === 'used' ? 'usado / rodado' : 'no indicada'}. Marca, modelo y versión: ${leasingVehicleDescription || 'no indicados'}. Año modelo: ${leasingVehicleYear || 'no indicado'}. Valuación fiscal: ${leasingFiscalValuation || 'no indicada'}. Patente total 2025: ${leasingPriorYearPatent || 'no indicada'}.`
      : '';
