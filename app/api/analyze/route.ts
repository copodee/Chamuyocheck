...
export async function handleAnalyzeRequest(req: Request) {
  try {
    ...
    const leasingJurisdictionContext = selectedCategory === 'leasing-specialist'
      ...
      : '';
    const leasingVehicleContext = selectedCategory === 'leasing-specialist' && [leasingVehicleDescription, leasingVehicleYear, leasingVehicleCondition, leasingFiscalValuation, leasingPriorYearPatent].some(Boolean)
      ? `Datos del automotor informados por el usuario: condición: ${leasingVehicleCondition === 'new' ? '0 km' : leasingVehicleCondition === 'used' ? 'usado / rodado' : 'no indicada'}. Marca, modelo y versión: ${leasingVehicleDescription || 'no indicados'}.${leasingVehicleCondition === 'used' ? ` Año modelo del usado: ${leasingVehicleYear || 'no indicado'}.` : ''} Valuación fiscal: ${leasingFiscalValuation || 'no indicada'}. Patente total 2025: ${leasingPriorYearPatent || 'no indicada'}.`
      : '';
