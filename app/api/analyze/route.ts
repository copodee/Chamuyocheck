...
function local(text:string,input:string){
 const fin=/(pr[eé]stamo|cuota|cft|tea|tna|cr[eé]dito|financiaci[oó]n|inter[eé]s|\$)/i.test(text);
 ...
 const categoryScores=[
  ...
  {name:'IA académica',score:academic?58:0,explanation:academic?'Estimación no concluyente: pedir defensa oral, borradores y fuentes.':'No parece un trabajo académico principal.'}
 ];
 return {score,risk:score>74?'Alto':score>44?'Medio':'Bajo',confidence:missing?'Media':'Alta',detectedTheme:theme,detectedInput:input,centralQuestion:fin?'¿Puedo decidir sin ver el costo total y el contrato?':academic?'¿Hay base suficiente para pedir más evidencia sin acusar?':'¿Puedo confiar en esto sin pedir más evidencia?',summary:fin?'El texto activa análisis financiero. La prioridad es costo total, CFT y cargos omitidos.':academic?'El análisis puede orientar una revisión docente, pero no prueba uso de IA ni plagio.':'El contenido requiere contrastar evidencia, fuentes y condiciones.',prudentConclusion:fin?'No decidiría por la cuota hasta ver CFT, contrato y cargos.':academic?'No acusaría; pediría explicación oral, borradores y fuentes.':'Pediría evidencia y condiciones antes de decidir.',verdict:'Evaluación prudente: indicadores de riesgo, no afirmación definitiva sobre veracidad, autoría, legalidad o intención.',categoryScores,modules:categoryScores.filter((c:any)=>c.score>0).slice(0,6),flaggedPhrases:promise?[{phrase:text.slice(0,220),problem:'Promesa fuerte o beneficio presentado con poca incertidumbre visible.',severity:'Media'}]:[],issues:[promise?'Promesa o resultado atractivo sin margen claro de incertidumbre.':'',missing?'Faltan fuentes o metodología verificable.':'',fin?'Faltan costos financieros completos.':'',pyramid?'Posible estructura basada en referidos o rentabilidad prometida.':''].filter(Boolean),questions:['¿Qué fuente independiente respalda la afirmación?','¿Qué condiciones deberían cumplirse?','¿Qué pasa si no se cumple?',fin?'¿Cuál es el CFT efectivo anual con IVA incluido?':'',academic?'¿El autor puede defender oralmente el trabajo y mostrar borradores?':'',pyramid?'¿El ingreso depende de sumar referidos o de vender un producto real?':''].filter(Boolean),missingInformation:['fuentes verificables','condiciones completas','metodología o base del dato',fin?'CFT, TEA, TNA, IVA, seguros, comisiones y mora':'',academic?'borradores, fuentes, historial y defensa oral':'',pyramid?'modelo de negocio y origen real de pagos':''].filter(Boolean),worstCase:fin?'Aceptar por la cuota y descubrir cargos o un CFT mayor.':pyramid?'Ingresar por rentabilidad y que dependa de sumar nuevos participantes.':'Tomar una decisión impulsiva con información incompleta.',improved:fin?'Mostrar monto neto, cuota, plazo, CFT con IVA, TEA, TNA, comisiones, seguros, gastos, mora y total a pagar.':'Explicar alcance, límites, requisitos, evidencia, costos, riesgos y condiciones verificables.',legalSafeguard:'ChamuyoCheck genera una evaluación automatizada basada exclusivamente en el contenido ingresado. No afirma que una persona o empresa mienta, no determina ilicitud, autoría ni plagio, y no reemplaza asesoramiento legal, financiero, médico, educativo ni profesional.'}
}
