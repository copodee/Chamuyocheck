...
export async function handleAnalyzeRequest(req: Request) {
  try {
    ...
    const leasingJurisdictionContext = selectedCategory === 'leasing-specialist'
      ? `Jurisdicción de celebración e instrumentación del contrato: ${leasingContractProvince || 'no indicada'}. Provincia de uso, guarda y radicación del bien: ${leasingProvince}. Domicilio del dador: ${leasingLessorProvince || 'no indicado'}.${leasingComparisonProvince ? ` Escenario provincial alternativo para comparar contrato y radicación: ${leasingComparisonProvince}.` : ''} Caso práctico: leasing financiero con sistema francés. Tipo de bien: ${leasingAssetType || 'no indicado'}. Valor del bien sin IVA: ${leasingAssetValue || 'no indicado'}. Porcentaje financiado: ${leasingFinancedPercent || 'no indicado'}%. Plazo: ${leasingMonths || 'no indicado'} meses. TNA: ${leasingTna || 'no indicada'}%. Modalidad de opción: ${leasingOptionMode === 'amount' ? 'importe fijo' : 'porcentaje del valor del bien'}. Opción de compra porcentual: ${leasingOptionMode === 'percent' ? (leasingOptionPercent || 'no indicada') : 'no aplica'}%. Opción de compra importe fijo: ${leasingOptionMode === 'amount' ? (leasingOptionAmount || 'no indicado') : 'no aplica'}. Cánones de garantía recibidos al inicio y aplicados a las últimas cuotas: ${leasingGuaranteeCanons || '0'}. Gasto de estructuración: ${leasingStructuringFeePercent || 'no indicado'}% del valor financiado. El valor base informado es neto de IVA; calcular y explicar IVA por separado sin incorporarlo dos veces al capital. Determinar territorialidad y pago de Sellos considerando ambas jurisdicciones, los efectos del instrumento y las reglas que eviten doble imposición. Comparar únicamente porcentajes, bases y condiciones —sin calcular montos— de Sellos, Ingresos Brutos, inscripción inicial, patentamiento, patente anual y opción de compra. Para el flujo financiero sí calcular monto financiado, aporte inicial, canon francés, opción, garantía, gasto y TIR del dador cuando existan datos suficientes. La garantía se recibe al inicio y se imputa contra las últimas cuotas: no duplicar esos cobros; impuestos y facturación se reconocen cuando corresponda según contrato y norma. Clasificar el cambio provincial como posible, condicionado a una conexión real o no justificable. Para Patentes CABA 2026 aplicar la Ley 6.926: domicilio del tomador, guarda habitual o uso/explotación; no usar el domicilio del dador. No se presume libre elección fiscal.`
      : '';
