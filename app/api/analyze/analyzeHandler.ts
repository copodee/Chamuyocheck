...
export async function handleAnalyzeRequest(req: Request) {
  try {
    ...
    const leasingVehicleCondition = String(form.get('leasingVehicleCondition') || '').trim();
    const leasingClientName = String(form.get('leasingClientName') || '').trim();
    const leasingAdvancePercent = String(form.get('leasingAdvancePercent') || '').trim();
    const leasingAdvanceAmount = String(form.get('leasingAdvanceAmount') || '').trim();
    const leasingFinancedAmount = String(form.get('leasingFinancedAmount') || '').trim();
    ...
    const leasingJurisdictionContext = selectedCategory === 'leasing-specialist'
      ? `Domicilio de uso del cliente: ${leasingProvince}. Jurisdicción legal de quien financia: ${leasingFinancierJurisdiction || 'no indicada'}.${leasingComparisonProvince ? ` Provincia alternativa de uso para comparar: ${leasingComparisonProvince}.` : ''} ${leasingQuoteUploaded ? 'Se adjuntó una cotización: extraer del documento valor neto e IVA, plazo, cantidad e importe de cánones, maxi canon, opción de compra, cánones de garantía, comisión, seguro y gastos. Esos datos prevalecen y no deben reemplazarse por valores predeterminados del formulario. Pedir sólo la información que realmente falte.' : `Caso práctico: leasing financiero con sistema francés. Cliente: ${leasingClientName || 'no indicado'}. Tipo de bien: ${leasingAssetType || 'no indicado'}. Valor del bien sin IVA: ${leasingAssetValue || 'no indicado'}. Anticipo: ${leasingAdvancePercent || 'no indicado'}% (${leasingAdvanceAmount || 'monto no indicado'}). Saldo financiado: ${leasingFinancedAmount || 'no indicado'} (${leasingFinancedPercent || 'no indicado'}%). Plazo: ${leasingMonths || 'no indicado'} meses. TNA: ${leasingTna || 'no indicada'}%. Modalidad de opción: ${leasingOptionMode === 'amount' ? 'importe fijo' : 'porcentaje del valor del bien'}. Opción de compra porcentual: ${leasingOptionMode === 'percent' ? (leasingOptionPercent || 'no indicada') : 'no aplica'}%. Opción de compra importe fijo: ${leasingOptionMode === 'amount' ? (leasingOptionAmount || 'no indicado') : 'no aplica'}. Cánones de garantía recibidos al inicio y aplicados a las últimas cuotas: ${leasingGuaranteeCanons || '0'}. Gasto de estructuración: ${leasingStructuringFeePercent || 'no indicado'}% del valor financiado.`} El valor base se trata como neto de IVA; calcular y explicar IVA por separado sin incorporarlo dos veces al capital. El seguro se modela por defecto como contratado y pagado por el dador, quien lo refactura mensualmente al tomador como concepto separado del canon financiero; si la cotización o el contrato indican otra mecánica, prevalece el documento. Determinar territorialidad y pago de Sellos considerando el domicilio de uso del cliente, la jurisdicción legal de quien financia, los efectos del instrumento y las reglas que eviten doble imposición. Comparar únicamente porcentajes, bases y condiciones —sin calcular montos— de Sellos, Ingresos Brutos, inscripción inicial, patentamiento, patente anual y opción de compra. Para el flujo financiero sí calcular monto financiado, aporte inicial, canon, opción, garantía, gasto y TIR del dador cuando existan datos suficientes. La garantía se recibe al inicio y se imputa contra las últimas cuotas: no duplicar esos cobros; impuestos y facturación se reconocen cuando corresponda según contrato y norma. Clasificar el cambio provincial como posible, condicionado a una conexión real o no justificable. Para patentes usá el valor neto declarado sólo como base preliminar y expresá la alícuota porcentual; no inventes un monto total. No se presume libre elección fiscal.`
      : '';
    const leasingVehicleContext = selectedCategory === 'leasing-specialist' && [leasingVehicleDescription, leasingVehicleYear, leasingVehicleCondition, leasingAssetValue].some(Boolean)
      ? `Datos del automotor informados por el usuario: condición: ${leasingVehicleCondition === 'new' ? '0 km' : leasingVehicleCondition === 'used' ? 'usado / rodado' : 'no indicada'}. Marca, modelo y versión: ${leasingVehicleDescription || 'no indicados'}.${leasingVehicleCondition === 'used' ? ` Año modelo del usado: ${leasingVehicleYear || 'no indicado'}.` : ''} Valor neto declarado: ${leasingAssetValue || 'no indicado'}. Si corresponde estimar Patentes, informar sólo la alícuota porcentual aplicada sobre esa base preliminar; si no está verificada, indicarlo.`
      : '';
...
export async function analyzePost(req: Request) {
