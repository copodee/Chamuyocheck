...
export async function verifyArgentinaCriminalLaw(
  ...
  const isArgentina = /\b(?:argentina|argentinas?|c[oó]digo\s+penal)\b/i.test(claimText);
  const isTheft = /\b(?:roba|robar|hurto|sustrae)\b/i.test(claimText);
  const isSexualOffense = /\b(?:violaci[oó]n|violador(?:a|es)?|abuso\s+sexual|acceso\s+carnal|integridad\s+sexual)\b/i.test(claimText);
  if (!isArgentina || (!isTheft && !isSexualOffense) || !claimIndexes.length) return none;
  try {
    ...
    const article = isSexualOffense
      ? body.match(/ARTICULO\s+119[\s\S]{0,2600}?ARTICULO\s+120/i)?.[0]
      : body.match(/ARTICULO\s+162[^.]*\.\s*-?\s*(El que se apoderare[\s\S]{0,500}?prisi[oó]n de un mes a dos años)/i)?.[0]
    ...
    const record: ExternalVerificationSourceRecord = {
      sourceType: 'government-law-repository', url: PENAL_CODE, title: `Código Penal de la Nación Argentina — Ley 11.179, artículo ${isSexualOffense ? '119' : '162'}`,
      retrievedAt, sourceDate: retrievedAt.slice(0, 10), claimIndexes: [...new Set(claimIndexes)], official: true, excerpt: article.slice(0, isSexualOffense ? 2200 : 700),
    };
    if (isSexualOffense) return { records: [record], assessment: 'inconclusive', rationale: 'Se consultó el artículo 119 vigente del Código Penal argentino. La norma contempla escalas distintas según la modalidad y los agravantes; sin hechos concretos no corresponde seleccionar una pena única.' };
