...
export function buildScoreExplanation(text: string, topic: string | undefined, inputKind: string, score: number, issues: string[], verification?: VerificationResult, reasoning?: ReasoningResult, universalReasoning?: UniversalReasoningResult, weightedResult?: DomainWeightedResult) {
  ...
  // Extraordinary claims detected by weighted scoring
  if (weightedResult?.isExtraordinaryClaim && !universalReasoning?.triggered && score >= 90 && score < 100) {
    items.push('⚠️ AFIRMACIÓN EXTRAORDINARIA SIN RESPALDO');
    items.push('Se describe un evento extraordinario y reciente (extraterrestres, fenómenos inexplicados, etc.).');
    items.push('Sin fuentes verificables, registro audiovisual o confirmación independiente.');
    items.push(`Dimensiones consideradas: ${weightedResult.applicableDimensions.map(d => d.label).join(', ')}.`);
    items.push(`Dimensiones excluidas como no aplicables: ${weightedResult.dimensions.filter(d => !d.applicable).map(d => d.label).join(', ')}.`);
    return items.slice(0, 8);
  }
  ...
  // Show only applicable dimensions in generic explanation
  if (weightedResult && weightedResult.applicableDimensions.length > 0) {
    items.push(`Dimensiones de análisis activas: ${weightedResult.applicableDimensions.slice(0, 3).map(d => d.label).join(', ')}.`);
  }
