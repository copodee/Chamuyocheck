...
drop policy if exists "administrators manage memberships" on public.memberships;
create policy "administrators manage memberships" on public.memberships
for all using (public.is_administrator(organization_id))
with check (public.is_administrator(organization_id));

drop policy if exists "members insert prequalifications" on public.prequalifications;
create policy "members insert prequalifications" on public.prequalifications
for insert with check (
  auth.uid() = created_by and public.is_active_member(organization_id)
);

drop policy if exists "members read prequalifications" on public.prequalifications;
create policy "members read prequalifications" on public.prequalifications
for select using (public.is_active_member(organization_id));
...
-- insert into public.organizations (name) values ('LeasingScoring Administración') returning id;
